...
class ClockThread(Thread):
	...
	def run(self):
		while self.alive:
			...
			if self.printFront: print(self.clock.value)
			time.sleep(self.spd)
	
	def pause(self): self.paused = True		# mettre en pause
	
	def unpause(self): self.paused = False	# reprendre
	
	def stop(self): self.alive = False		# arrêter
	
	def speed(self, sec): self.spd = sec	# changer la vitesse (en sec)
